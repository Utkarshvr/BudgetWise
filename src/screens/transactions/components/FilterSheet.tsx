...
export function FilterSheet({
  ...
  const [tempSelectedAccountIds, setTempSelectedAccountIds] = useState<string[]>(selectedAccountIds);
  const [tempSelectedCategoryIds, setTempSelectedCategoryIds] = useState<string[]>(selectedCategoryIds);
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const errorMessage = getErrorMessage(error, "Failed to fetch filter data");
    ...
    if (activeTab === "account") {
      ...
      const relevantCategories =
        activeTab === "income"
          ? incomeCategories
          : expenseCategories;
      const othersId = activeTab === "income" ? "others_income" : "others_expense";
      const hasOthers = tempSelectedCategoryIds.includes(othersId);
      const hasOthersTotal = categoryTotals[othersId] && categoryTotals[othersId] > 0;
      ...
      const allSelected = allCategoryIds.every((id) => tempSelectedCategoryIds.includes(id));
      
      if (allSelected) {
        // Deselect all
        setTempSelectedCategoryIds(
          tempSelectedCategoryIds.filter(
        ...
        const newIds = [
          ...
          ...allCategoryIds.filter((id) => !tempSelectedCategoryIds.includes(id)),
        ];
    ...
    transactions.forEach((transaction) => {
      ...
      if (transaction.type === "income" && transaction.to_account_id) {
        if (!totals[transaction.to_account_id]) {
          totals[transaction.to_account_id] = { income: 0, expense: 0, currency };
        }
        totals[transaction.to_account_id].income += transaction.amount;
      } else if (transaction.type === "expense" && transaction.from_account_id) {
        if (!totals[transaction.from_account_id]) {
          totals[transaction.from_account_id] = { income: 0, expense: 0, currency };
        }
  ...
  const summaryTotals = useMemo(() => {
    ...
    const hasFilters = tempSelectedAccountIds.length > 0 || tempSelectedCategoryIds.length > 0;
    ...
    const filteredTransactions = transactions.filter((transaction) => {
      ...
      if (tempSelectedCategoryIds.length > 0) {
        const hasOthersIncome = tempSelectedCategoryIds.includes("others_income");
        const hasOthersExpense = tempSelectedCategoryIds.includes("others_expense");
    ...
    const incomePercent =
      allTotals.income > 0
        ? (filteredIncome / allTotals.income) * 100
        : 0;
    const expensePercent =
      allTotals.expense > 0
        ? (filteredExpense / allTotals.expense) * 100
        : 0;
  ...
  }, [transactions, tempSelectedAccountIds, tempSelectedCategoryIds, allTotals]);
  ...
  const isDirty = useMemo(() => {
    ...
    const categoriesChanged =
      ...
      !tempSelectedCategoryIds.every((id) => selectedCategoryIds.includes(id)) ||
      !selectedCategoryIds.every((id) => tempSelectedCategoryIds.includes(id));
    
    return accountsChanged || categoriesChanged;
  }, [tempSelectedAccountIds, tempSelectedCategoryIds, selectedAccountIds, selectedCategoryIds]);

  // Group accounts by type
  const groupedAccounts = useMemo(() => {
    const grouped: Record<string, Account[]> = {};
    accounts.forEach((account) => {
      const type = account.type === "cash" ? "Cash" : account.type === "checking" || account.type === "savings" ? "Accounts" : "Card";
      if (!grouped[type]) {
        grouped[type] = [];
      }
  ...
  const isAllSelected = useMemo(() => {
    if (activeTab === "account") {
      return tempSelectedAccountIds.length === accounts.length && accounts.length > 0;
    } else {
      const relevantCategories =
  ...
  }, [activeTab, tempSelectedAccountIds, tempSelectedCategoryIds, accounts, incomeCategories, expenseCategories, categoryTotals]);
        ...
        >
          ...
          <TouchableOpacity
            ...
            style={{
              ...
              borderBottomColor:
                activeTab === "expense" ? colors.primary.DEFAULT : "transparent",
            }}
          >
            <Text
              style={{
          ...
          <TouchableOpacity
            ...
            style={{
              ...
              borderBottomColor:
                activeTab === "account" ? colors.primary.DEFAULT : "transparent",
            }}
          >
            <Text
              style={{
                    ...
                    Object.entries(groupedAccounts)
                      ...
                      <View key={groupName}>
                        ...
                        <View style={{ gap: 8 }}>
                          ...
                          const isSelected = tempSelectedAccountIds.includes(account.id);
                          ...
                          return (
                            ...
                            >
                              ...
                              >
                                ...
                                >
                                  {formatAmount(totals.income, totals.currency)}
                                </Text>
                                <Text
                                  style={{
                                    fontSize: 14,
                                ...
                                >
                                  {formatAmount(totals.expense, totals.currency)}
                                </Text>
                              </View>
                            </TouchableOpacity>
                          );
